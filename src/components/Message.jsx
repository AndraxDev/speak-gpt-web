...
function Message({isBot, message, image, isAssistant, assistantName="SpeakGPT", assistantIcon = "/logo192.png", ...props}) {
    const randomMessageUUID = uuidv4();

    const [copyIcon, setCopyIcon] = React.useState(<ContentCopy fontSize={"small"}/>);

    return (
        <div className={"message " + (isBot ? "message-bot-bg" : "")}>
            { isBot ? (
                <div className={"message-bot"}>
                    <div className={"message-author"}>
                        <img alt={""} className={"message-icon material-symbols-outlined"} src={assistantIcon}/>
                        <b className={"message-author-2"}>{assistantName}</b>
                    </div>
                    {!message.toString().includes("~file:") ? <Markdown
                            ...
                            className={isMobile ? "message-content-mob" : "message-content"}>{message}</Markdown> :
                        <img alt={"Generated output"} id={"image-" + sha256(message)} className={isAssistant || isMobile ? "chat-image-assistant" : "chat-image"}
                    ...
                    <div className={"message_buttons"}>
                        <button className={"btn_copy"} id={"btn_copy_" + randomMessageUUID} onClick={() => {
                            const text = message.toString().includes("~file:") ? message.toString().replace("~file:", "") : message;
                            navigator.clipboard.writeText(text).then(() => {
                                const copyButton = document.getElementById("btn_copy_" + randomMessageUUID);
                                copyButton.classList.add("btn_copy_success");
                                setCopyIcon(<Check fontSize={"small"}/>);
                                setTimeout(() => {
                                    copyButton.classList.remove("btn_copy_success");
                                    setCopyIcon(<ContentCopy fontSize={"small"}/>)
                                }, 2000);
                            }).catch(err => {
                            ...
                            }} className={"material-symbols-outlined"} id={"btn_copy_span_" + randomMessageUUID}>{copyIcon}</div>
                        </button>
                    </div>
                </div>
            ) : (
                <div className={"message-user"}>
                    <div className={"message-author"}>
                        <span className={"message-icon material-symbols-outlined"}><AccountCircle/></span>
                        <b className={"message-author-2"}>You</b>
                    </div>
                    <p className={"message-content"}>{message}</p>
                    {
                        image !== null && image !== undefined ?
                            <img alt={"Generated output"} id={"image-" + sha256(image.toString())} className={isAssistant || isMobile ? "chat-image-assistant" : "chat-image"} src={image}/> : null
                    }
                </div>
            )}
        </div>
