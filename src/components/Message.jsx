...
function Message({isBot, message, image, isAssistant, assistantName="SpeakGPT", assistantIcon = "/logo192.png", ...props}) {
    ...
    const highlightCode = (code, lang, l) => {
        try {
            let out = Prism.highlight(code, lang, l);
            return out;
        } catch (e) {
            return null;
        }
            ...
            { isBot ? (
                <div className={"message-bot"}>
                    <div className={"message-author"}>
                        <img className={"message-icon material-symbols-outlined"} src={assistantIcon}/>
                    ...
                    {!message.toString().includes("~file:") ? <Markdown
                        ...
                        <img id={"image-" + sha256(message)} className={isAssistant || isMobile ? "chat-image-assistant" : "chat-image"}
                ...
                <div className={"message-user"}>
                    ...
                    <p className={"message-content"}>{message}</p>
                    {
                        image !== null && image !== undefined ?
                            <img id={"image-" + sha256(image.toString())} className={isAssistant || isMobile ? "chat-image-assistant" : "chat-image"} src={image}/> : null
                    }
                </div>
            )}
        </div>
