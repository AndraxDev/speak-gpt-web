                    ...
                    <div className={"message_buttons"}>
                        <button className={"btn_copy"} id={"btn_copy_" + randomMessageUUID} onClick={() => {
                            ...
                            navigator.clipboard.writeText(text).then(() => {
                                const copyButtonSpan = document.getElementById("btn_copy_span_" + randomMessageUUID);
                                const copyButton = document.getElementById("btn_copy_" + randomMessageUUID);
                                copyButton.classList.add("btn_copy_success");
                                copyButtonSpan.innerText = "check";
                                setTimeout(() => {
                                    copyButton.classList.remove("btn_copy_success");
                                    copyButtonSpan.innerText = "content_copy";
                                }, 2000);
                            }).catch(err => {
                                console.error('Failed to copy text: ', err);
                            });
                        }}>
                            <span className={"material-symbols-outlined"} id={"btn_copy_span_" + randomMessageUUID}>content_copy</span>
                        </button>
                    </div>
                </div>
            ) : (
